...
const TransactionsHistory = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8000/api.php?controller=Payments&action=getPayments", {
      credentials: "include",
    })
            ...
            {paginatedPayments.length > 0 ? (
              paginatedPayments.map((tx) => (
                <tr key={tx.payment_id} className="hover:bg-gray-100 transition">
                  <td className="px-4 py-3">{tx.payment_id}</td>
                  ...
                  <td className="px-4 py-3">{tx.booking_status}</td>
                  <td className="px-4 py-3">{new Date(tx.payment_date).toLocaleString()}</td>
